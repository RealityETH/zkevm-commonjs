    ...
    it('Check test vectors', async () => {
        for (let i = 0; i < testVectors.length; i++) {
            ...
            for (let j = 0; j < txs.length; j++) {
                ...
                let customRawTx;
                // private key for the address '0x30cEE8B78e4a1cbBfd5Bd7867531bcaBdb00d581'. Only used for testing purposes
                const txPrivateKey = '0x57b0ab5149806bf26aa39ca1e266a7ed17afbea9d3f6acd653dc2bd14ef2b4ae';
                const wallet = new ethers.Wallet(txPrivateKey);
                if (tx.chainId === 0) {
                    ...
                    const signingKey = new ethers.utils.SigningKey(txPrivateKey);
            ...
            let batch = await zkEVMDB.buildBatch(timestamp, sequencerAddress, smtUtils.stringToH4(globalExitRoot));
            ...
            let updatedAccounts = batch.getUpdatedAccountsBatch();
            let newLeafs = {};
            ...
            let addressInstanceGlobalExitRoot = new Address(toBuffer(Constants.ADDRESS_GLOBAL_EXIT_ROOT_MANAGER_L2));
            let localExitRootPosBuffer = toBuffer(ethers.utils.hexZeroPad(Constants.LOCAL_EXIT_ROOT_STORAGE_POS, 32));
            let globalExitRootPos = ethers.utils.solidityKeccak256(['uint256', 'uint256'], [globalExitRoot, Constants.GLOBAL_EXIT_ROOT_STORAGE_POS]);
            let globalExitRootPosBuffer = toBuffer(globalExitRootPos);

            // Check local exit root
            let localExitRootVm = await zkEVMDB.vm.stateManager.getContractStorage(addressInstanceGlobalExitRoot, localExitRootPosBuffer);
            let localExitRootSmt = (await stateUtils.getContractStorage(
            ...
            let timestampVm = await zkEVMDB.vm.stateManager.getContractStorage(
                addressInstanceGlobalExitRoot,
                globalExitRootPosBuffer,
            );
            let timestampSmt = (await stateUtils.getContractStorage(
                Constants.ADDRESS_GLOBAL_EXIT_ROOT_MANAGER_L2,
                zkEVMDB.smt,
                zkEVMDB.stateRoot,
                [globalExitRootPos],
            ...
            let circuitInput = await batch.getStarkInput();
            ...
            snarkInput = await batch.getStarkInput();
        ...
        console.log("second update to the chains with tx2s");
            ...
            batch = await zkEVMDB.buildBatch(timestamp, sequencerAddress, smtUtils.stringToH4(globalExitRoot));
            for (let j = 0; j < rawTxs.length; j++) {
                batch.addRawTx(rawTxs[j]);
            }
            ...
            newRoot = batch.currentStateRoot;
            if (!update) {
                expect(smtUtils.h4toString(newRoot)).to.be.equal(expectedNewRoot);
                expect(smtUtils.h4toString(batch.newAccInputHash)).to.be.equal(expectedNewAccInputHash);
            } else {
            ...
            // Check balances and nonces
             updatedAccounts = batch.getUpdatedAccountsBatch();
             newLeafs = {};
            for (const item in updatedAccounts) {
                const address = item;
                const account = updatedAccounts[address];
            ...
            // Check global and local exit roots
             addressInstanceGlobalExitRoot = new Address(toBuffer(Constants.ADDRESS_GLOBAL_EXIT_ROOT_MANAGER_L2));
             localExitRootPosBuffer = toBuffer(ethers.utils.hexZeroPad(Constants.LOCAL_EXIT_ROOT_STORAGE_POS, 32));
             globalExitRootPos = ethers.utils.solidityKeccak256(['uint256', 'uint256'], [globalExitRoot, Constants.GLOBAL_EXIT_ROOT_STORAGE_POS]);
            globalExitRootPosBuffer = toBuffer(globalExitRootPos);

            // Check local exit root
             localExitRootVm = await zkEVMDB.vm.stateManager.getContractStorage(addressInstanceGlobalExitRoot, localExitRootPosBuffer);
             localExitRootSmt = (await stateUtils.getContractStorage(
                Constants.ADDRESS_GLOBAL_EXIT_ROOT_MANAGER_L2,
                zkEVMDB.smt,
                zkEVMDB.stateRoot,
                [Constants.LOCAL_EXIT_ROOT_STORAGE_POS],
            ...
            // Check global exit root
             timestampVm = await zkEVMDB.vm.stateManager.getContractStorage(
                addressInstanceGlobalExitRoot,
                globalExitRootPosBuffer,
            );
             timestampSmt = (await stateUtils.getContractStorage(
                Constants.ADDRESS_GLOBAL_EXIT_ROOT_MANAGER_L2,
                zkEVMDB.smt,
                zkEVMDB.stateRoot,
                [globalExitRootPos],
            ...
            // Check the circuit input
           circuitInput = await batch.getStarkInput();
